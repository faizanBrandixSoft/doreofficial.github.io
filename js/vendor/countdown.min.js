  ...
  (b.prototype.initialize = function () {
    ...
    (b.prototype.run = function () {
      var b,
        c = this,
        d = Math.abs(
          this.seconds(this.conf.dateEnd) - this.seconds(new Date())
        );
        ...
        (b = a.setInterval(function () {
          d--,
            0 >= d
              ...
              : c.isStarted() &&
                (c.started || (c.callback("start"), (c.started = !0)),
                c.display(d));
        }, this.interval));
    }),
    (b.prototype.display = function (a) {
      ...
      for (var g = 0; g < this.selector.length; g++)
        this.selector[g].innerHTML = b;
    }),
    (b.prototype.defineInterval = function () {
      for (var a = this.patterns.length; a > 0; a--) {
        var b = this.patterns[a - 1];
        if (-1 !== this.conf.msgPattern.indexOf(b.pattern))
          return void (this.interval = 1e3 * b.secs);
      }
    }),
    (b.prototype.outOfInterval = function () {
      for (
        var a =
            new Date() < this.conf.dateStart
              ? this.conf.msgBefore
              : this.conf.msgAfter,
          b = 0;
        b < this.selector.length;
        b++
      )
    ...
    (b.prototype.callback = function (b) {
      ...
      "function" == typeof this.conf["on" + c] && this.conf["on" + c](),
        "undefined" != typeof a.jQuery &&
          a.jQuery(this.conf.selector).trigger("countdown" + c);
    }),
